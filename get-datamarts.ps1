...
function Get-Datamarts {
    ...
    begin
    {
        $rawMarts = if($dataMartIds){ Get-DosData -Uri "$($metadataServiceUrl)/DataMarts" | Where-Object {$_.Id -in $dataMartIds} }else{ Get-DosData -Uri "$($metadataServiceUrl)/DataMarts" };
